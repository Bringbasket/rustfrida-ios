...
| P2-H（静态 transform/relocation/layout plan 已完成） | 指令级 Stalker 核心 | `$I/native-api/src/stalker.rs`、`$I/quickjs-runtime/src/{arm64_relocator.rs,native.rs,stalker.rs}` | bounded basic-block transform、caller-supplied execution event generation、direct-only ARM64 relocation plan、static code-cache block/island layout 和 QuickJS 接口已完成；code-cache materialization、writer emission、RX mapping、真实 target-thread transformer/instruction rewrite 和 event sink backend 待实现 |
