...
3. `Memory`、`Module`、`Interceptor`、按进程名 Attach、`NativeFunction` 标量 ABI、native pointer hook、suspended spawn 生命周期、multi-session/server/断线回收均已静态接线。Simulator `simctl --wait-for-debugger` bundle gate、Stalker bounded static transform/event generation、ObjC typed/custom/atomic/KVO property 与有界 heap choose、Swift ABI 分类及 object ownership/dispose/metadata identity 验证边界、CModule Apple TinyCC/Mach-O/MAP_JIT backend 和外部 adapter FFI execution boundary 也已接线；agent-owned external-hook command/receipt 路径以及 controller remote lease 的远程 command/receipt 转移与产品级调用链已完成源码/静态接线，待 Apple runtime/真机验收。剩余边界是物理设备 FrontBoard/scene provider、真正的 target-thread 指令级 Stalker、Swift full object ABI、Apple `MAP_JIT` 运行验收和 controller remote lease 的 Apple runtime/真机行为。
...
整体判定：**核心 Attach/QuickJS/Apple 元数据能力已经形成，但 iOS 尚未达到 Android 全功能 parity。本轮 Stalker 静态 transform/event generation、Swift object ownership/dispose/metadata identity、agent-owned external-hook command/receipt 状态和 controller remote lease 管理 API、远程 command/receipt 转移与产品级调用链已完成源码/静态接线，待 Apple runtime/真机验收。生产执行路径仍需物理设备 v2 runtime-dynamic bundle gate、真正的 target-thread Stalker、Swift full object ABI、Apple host/device 的 CModule `MAP_JIT` 运行验收，因此 external adapter 继续标记为 integration boundary，而非生产运行验收完成。**
...
- iOS 基线：`$I`，Git HEAD `d47c56f6954ac73f820da4bd76c8661721bba2f7`，按 `HEAD + working tree` 审计。
- Android 基线：`$A`，Git HEAD `1d9e2ac4e930fe2c22f5b15e1893c9aa2665d10c`，审计时工作区干净。
...
| Mach-O 原生查询 | `complete` | `$A/quickjs-hook/src/jsapi/module/elf.rs`；`enumerate.rs` | `$I/quickjs-runtime/src/native.rs`；`$I/native-api/src/{exports,imports,symbols,segments,sections,loadcmds,chained_fixups,code_signature}.rs` | iOS 已覆盖 image、symbol、export/import、依赖、加密、入口、dyld/linkedit、function starts、签名、trie/fixups、版本、UUID、rpath、segment/section/load command 等。 |
...
| `CModule` | `complete` | `$A/quickjs-hook/src/jsapi/hook_api/cmodule.rs` | `$I/quickjs-runtime/src/cmodule.rs`；`runtime.rs`；`cmodule-src/` | Apple backend 已接入 TinyCC compiler、Mach-O linker、`MAP_JIT` executable mapping、imports/exports、`findSymbol`、finalizer 和 `dropMetadata`，并保留源码/import 的真实校验。`quickjs_cmodule` x86_64 host forced-backend smoke 为 7/7，导出函数实际返回 `42`，ARM64 codegen C 编译通过；Apple host/device `MAP_JIT` 运行验收待完成，Linux 普通构建按平台返回 `unavailable`。 |
...
| Frida Gum 指令级 Stalker | `partial` | `$A/agent/src/stalker.rs`；`$A/frida-gum/src/stalker.rs`；`$A/agent/src/recompiler.rs` | `$I/native-api/src/stalker.rs`；`$I/quickjs-runtime/src/{native.rs,native_hooks.rs,stalker.rs}` | iOS 已有事件/mask、排除区间、生命周期、有限队列和全局 `Stalker.capabilities/status` API；本轮新增 bounded basic-block static transform、caller-supplied execution trace 的 `Compile/Block/Exec/Call/Ret` event generation，以及 QuickJS `transform/transformBasicBlock/generateEvents/recordBlock` 接线。能力报告明确 `transformMode=static-only`、`eventGenerationMode=caller-supplied-execution-trace`，`targetThreadInstrumentation/targetThreadInstructionRewrite/instrumented=false`；真实 target-thread follow/unfollow、指令重写、transformer/callout、生产 event sink 和 instruction event backend 仍未接入。 |
...
2. **P2：实现真正指令级 Stalker backend。** bounded basic-block transform plan、caller-supplied execution event generation 和 QuickJS 接口已完成源码/静态接线；还需要真实 target-thread follow/unfollow、ARM64 instruction rewrite/code cache、生产 transformer/callout、event sink 和 flush/GC backend。
...
4. **P2：完成 CModule 与 external adapter 的验收/产品运行验证。** CModule 的 Apple TinyCC compiler、Mach-O linker、`MAP_JIT` mapping、imports/exports、`findSymbol`、finalizer 和 `dropMetadata` 已实现，host forced-backend smoke 7/7；剩余是 Apple host/device `MAP_JIT` 运行验收，Linux 普通构建的 `unavailable` 属于平台边界。agent-owned external-hook command/receipt path、controller install/replace remote lease、receipt 转移和产品级调用链已完成源码/静态接线，待 Apple runtime/真机验收；生产 token 回收时序仍需 Apple runtime 验证。四种 backend 未公开 native uninstall 的边界继续保持明确。QBDI 保持独立产品决策。
...
当前工作树测试计数记录：`controller` 256、`common` 13、`agent` 6、`native-api` 165、`objc-api` 48、`quickjs-runtime` 80，workspace 全量 host 测试 `568` 项全部通过。此前记录的 `563`（其中 `controller` 251）、`537`（其中 `controller` 229）和更早的 `532`（其中 `controller` 224）均为旧阶段快照。Apple cross-check `5/5`、CModule forced-backend smoke `7/7`，x86_64 host 导出函数实际返回 `42`，ARM64 codegen 的 C 编译通过，均属于既有 host/static 与交叉编译门禁；Apple host/device 的 `MAP_JIT`、真实 provider、注入、hook、KVO、Swift ownership、controller remote lease 和 agent adapter 生产调用时序仍按下一节验收。
...
4. arm64e/PAC 指针规范化、`pthread_create_from_mach_thread` fallback、CModule `MAP_JIT`/W^X 行为和真实 executable mapping。
...
仓库 CI `$R/.github/workflows/ios-rustfrida-ci.yml` 当前覆盖 Linux host test、macOS host test、iOS simulator `cargo check`、device agent build 和 macOS controller build；没有 simulator runtime integration，也没有越狱设备注入/hook test。因此 CI 绿色仍不能关闭上述项目。
...
| P2-H（静态 transform 已完成） | 指令级 Stalker 核心 | `$I/native-api/src/stalker.rs`、`$I/quickjs-runtime/src/{native.rs,stalker.rs}` | bounded basic-block transform、caller-supplied execution event generation 和 QuickJS 接口已完成；真实 target-thread transformer、instruction rewrite、event sink backend 待实现 |
...
| P2-K（能力边界已完成） | CModule / 外部 adapter | `$I/quickjs-runtime/src/cmodule.rs`、`$I/quickjs-runtime/cmodule-src/`、`$I/native-api/src/hook_backend_adapter.rs`、`$I/native-api/src/hook_backend_ffi.rs`、`$I/controller/src/{session.rs,server.rs,injection.rs}` | CModule Apple TinyCC/Mach-O/MAP_JIT compiler/linker backend、imports/exports/findSymbol/finalizer/dropMetadata、adapter decision boundary、token/lease/release、agent-owned external-hook command/receipt、controller adopt/release/uninstall/cleanup API、`Session::execute_external_hook`、`RemoteExternalHookLease`、`SessionRegistry::execute_external_hook` 和 Apple external install/replace FFI 已完成源码/静态接线；controller remote lease、产品级调用链和 controller tests 已完成源码/静态接线，待 Apple runtime/真机验收，生产 token 回收时序仍需 Apple runtime 验证；CModule Apple host/device runtime 验收仍是 integration boundary；四种已确认 ABI 的 native uninstall 保持明确 unavailable |
