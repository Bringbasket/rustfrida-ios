...
2. **P2：实现真正指令级 Stalker backend。** bounded basic-block transform plan、caller-supplied execution event generation、direct-only ARM64 relocation plan、static code-cache block/island layout、离线 writer emission、RW materialization、`Stalker.finalizeCodeCache()` 的 flush+RX transition 和 QuickJS owner/dispose/finalizer 接口已完成源码/静态接线；还需要真实 target-thread instruction rewrite、生产 transformer/callout、event sink 和 flush/GC backend；followed-thread static execution plan 与 current-thread direct execution 都不替代 target-thread instrumentation。
