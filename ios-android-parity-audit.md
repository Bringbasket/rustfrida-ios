...
3. `Memory`、`Module`、`Interceptor`、按进程名 Attach、`NativeFunction` 标量 ABI、native pointer hook、suspended spawn 生命周期、multi-session/server/断线回收均已静态接线。Simulator `simctl --wait-for-debugger` bundle gate、Stalker bounded static transform/event generation/direct relocation/layout/offline writer emission/RW materialization、flush+RX finalize、AArch64 current-thread direct execution、ObjC typed/custom/atomic/KVO property 与有界 heap choose、Swift ABI 分类及 object ownership/dispose/metadata identity 验证边界、CModule Apple TinyCC/Mach-O backend 和外部 adapter FFI execution boundary 也已接线；其中 CModule JIT 已在 ARM64 macOS host 和 ARM64 iOS Simulator 完成真实运行验收。agent-owned external-hook command/receipt 路径以及 controller remote lease 的远程 command/receipt 转移与产品级调用链已完成源码/静态接线，待 Apple runtime/真机验收。剩余边界是物理设备 FrontBoard/scene provider、真正的 target-thread 指令级 Stalker、Swift full object ABI、物理设备的 CModule `MAP_JIT`/W^X 行为和 controller remote lease 的 Apple runtime/真机行为。
...
| Frida Gum 指令级 Stalker | `partial` | `$A/agent/src/stalker.rs`；`$A/frida-gum/src/stalker.rs`；`$A/agent/src/recompiler.rs` | `$I/native-api/src/stalker.rs`；`$I/quickjs-runtime/src/{arm64_relocator.rs,native.rs,native_hooks.rs,stalker.rs}` | iOS 已有事件/mask、排除区间、生命周期、有限队列和全局 `Stalker.capabilities/status` API；本轮新增 bounded basic-block static transform、caller-supplied execution trace 的 `Compile/Block/Exec/Call/Ret` event generation、QuickJS `transform/transformBasicBlock/generateEvents/recordBlock` 接线，以及复用现有 ARM64 writer/relocator 的 `Stalker.relocate` direct-only 静态 relocation plan。`Stalker.layoutCodeCache` 进一步按终止指令切 basic blocks，并只规划 code 区、16-byte 对齐 fallback island 区与 64-byte reservation slots；结果固定 `materialized=false / executable=false / output=null`。`Stalker.emitCodeCache` 新增独立的离线 writer emission：direct 项写 relocated word，超范围 branch、ADR/ADRP、GPR/FP literal 和 PRFM 通过相邻 slot trampoline 发射，未使用字节以 NOP 填充；结果提供 output bytes，但仍固定 `executionReady=false / materialized=false / executable=false / allocatesExecutableMemory=false`，并显式报告 AAPCS64 `x16/IP0` veneer scratch 边界。`Stalker.materializeCodeCache(bytes, source)` 按最坏空间预估和 page alignment 分配匿名 RW mapping，以真实 mapping base 重新 emission、校验并复制 image；QuickJS owner 提供幂等 `dispose()`，finalizer 兜底 `munmap`。新增 `Stalker.finalizeCodeCache(cache)`：先 flush instruction cache，再以事务式 page protection 将完整 mapping 切换到 `r-x`，重复调用幂等；Apple mapping 使用 `MAP_JIT` 标志。结果在 finalize 前为 `materialized=true / executable=false / executionReady=false / mappingProtection=rw-`，成功后为 `executable=true / codeCacheWritableMapping=false / mappingProtection=r-x / executionReady=false`。新增 `Stalker.executeCodeCache(cache[, entryOffset])`，仅在 AArch64 finalized mapping 上执行无参数当前线程入口，默认返回 `{executionMode:"current-thread-direct", targetThread:false}` 结果；非 AArch64 返回稳定 unsupported。能力报告另提供 `staticCodeCacheFinalization=true`、`codeCacheFinalizationMode=flush-and-rx`、`staticCodeCacheDirectExecution` 与 `codeCacheDirectExecutionMode`，并保留 `codeCacheExecutionReady=false`。finalization 后新增 AArch64-only 的 `Stalker.executeCodeCache(cache[, entryOffset])` 当前线程直接入口调用，要求 mapping 已切换到 `r-x`；该调用返回显式 `executionMode=current-thread-direct`，但不宣称 `executionReady` 或 target-thread instrumentation。整体范围只处理 caller-supplied ARM64 bytes/trace；不修改目标代码，也不执行 target-thread instrumentation；target-thread follow/unfollow、指令重写、transformer/callout、生产 event sink 和 instruction event backend 仍未接入。 |
...
2. **P2：实现真正指令级 Stalker backend。** bounded basic-block transform plan、caller-supplied execution event generation、direct-only ARM64 relocation plan、static code-cache block/island layout、离线 writer emission、RW materialization、`Stalker.finalizeCodeCache()` 的 flush+RX transition 和 QuickJS owner/dispose/finalizer 接口已完成源码/静态接线；还需要真实 target-thread follow/unfollow、ARM64 instruction rewrite、生产 transformer/callout、event sink 和 flush/GC backend；current-thread direct execution 不替代 target-thread instrumentation。
...
| P2-H（静态 transform/relocation/layout/emission、RW materialization 与 flush+RX finalize 已完成） | 指令级 Stalker 核心 | `$I/native-api/src/stalker.rs`、`$I/quickjs-runtime/src/{arm64_relocator.rs,native.rs,stalker.rs}` | bounded basic-block transform、caller-supplied execution event generation、direct-only ARM64 relocation plan、static code-cache block/island layout、离线 writer emission、RW mapping、真实 mapping base 重发射、`Stalker.finalizeCodeCache()` flush+RX transition、AArch64 `Stalker.executeCodeCache()` current-thread direct execution 以及 QuickJS owner/dispose/finalizer 已完成；真实 target-thread transformer/instruction rewrite 和 event sink backend 待实现 |
