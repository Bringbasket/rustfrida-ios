...
2. **P2：完成生产指令级 Stalker backend。** bounded transform、caller-supplied event generation、relocation/layout/emission、RW/RX cache、current-thread direct execution，以及 target-thread near prepare、fall-through continuation、完整 source snapshot 校验和可回滚入口 rewrite 已完成源码/静态接线；还需要 `BL/BLR` 的 LR 保留、生产 transformer/callout、自动 event sink/instruction event backend 与 flush/GC 执行闭环。当前 rewrite 事务仍不等价于持续 target-thread instrumentation。
