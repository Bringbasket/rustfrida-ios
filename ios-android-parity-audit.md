...
整体判定：**核心 Attach/QuickJS/Apple 元数据能力已经形成，但 iOS 尚未达到 Android 全功能 parity。CModule JIT 已在 ARM64 macOS host 与 ARM64 iOS Simulator 完成真实 executable mapping、导出执行与释放验收；物理设备 runtime 仍需单独验收。本轮 Stalker 静态 transform/event generation/direct relocation/layout/offline writer emission/RW materialization、flush+RX finalize 和 AArch64 current-thread direct execution、Swift object ownership/dispose/metadata identity、agent-owned external-hook command/receipt 状态和 controller remote lease 管理 API、远程 command/receipt 转移与产品级调用链已完成源码/静态接线，待 Apple runtime/真机验收。生产执行路径仍需物理设备 v2 runtime-dynamic bundle gate、真正的 target-thread Stalker、Swift full object ABI；external adapter 继续标记为 integration boundary，而非生产运行验收完成。**
...
| Frida Gum 指令级 Stalker | `partial` | `$A/agent/src/stalker.rs`；`$A/frida-gum/src/stalker.rs`；`$A/agent/src/recompiler.rs` | `$I/native-api/src/stalker.rs`；`$I/quickjs-runtime/src/{arm64_relocator.rs,memory.rs,native.rs,native_hooks.rs,stalker.rs}` | iOS 已有事件/mask、排除区间、生命周期、有限队列和全局 `Stalker.capabilities/status` API；已完成 bounded static transform、caller-supplied event generation、direct relocation、code-cache layout/emission/fallback island、RW materialization、flush+RX finalize 和 AArch64 current-thread direct execution。target-thread 路径现已包含 followed-thread rewrite plan、near RX prepare、commit preflight 与 suspend/quiesce/4-byte entry patch/resume/rollback owner；非终止 block 追加 `B sourceEnd`，条件分支保留 taken 与 fall-through 两条路径，末条 `B/BR/RET` 不追加 continuation，提交前比较完整 prepared source snapshot。普通 `layoutCodeCache/emitCodeCache/materializeCodeCache` 输出保持原语义。新增 `Stalker.installCallout(thread, target[, stealth])`，在 ARM64 hook engine 下提供函数级 native `Call`/`Ret` 事件并写入 bounded sink；返回显式 detach handle。仍缺 `BL/BLR` 的 LR 保留、生产指令级 transformer、自动 instruction-event backend 和 Apple 真机运行验收，因此整体保持 `partial`。 |
...
2. **P2：完成生产指令级 Stalker backend。** bounded transform、caller-supplied event generation、relocation/layout/emission、RW/RX cache、current-thread direct execution，以及 target-thread near prepare、fall-through continuation、完整 source snapshot 校验和可回滚入口 rewrite 已完成源码/静态接线；函数级 `Stalker.installCallout()` 已在 ARM64 hook engine 下把 native `Call`/`Ret` 送入 bounded sink，并要求显式 detach。还需要 `BL/BLR` 的 LR 保留、生产指令级 transformer、自动 event sink/instruction event backend 与 flush/GC 执行闭环。当前 rewrite 事务和函数级 callout 都不等价于持续 target-thread 指令级 instrumentation。
...
| P2-H（target rewrite 事务已闭合，生产 instrumentation 待完成） | 指令级 Stalker 核心 | `$I/native-api/src/stalker.rs`、`$I/quickjs-runtime/src/{arm64_relocator.rs,memory.rs,native.rs,stalker.rs}` | bounded transform、caller-supplied events、relocation/layout/emission/fallback island、RW/RX mapping、current-thread execution、followed-thread plan，以及 target-thread near prepare/preflight/commit/rollback 已完成；fall-through continuation 与完整 source snapshot 已验证。函数级 `Stalker.installCallout()` 已提供 ARM64 native `Call`/`Ret` sink，但生命周期由显式 detach handle 管理。`BL/BLR` LR 保留、生产指令级 transformer、自动 event sink/instruction event backend 和 Apple runtime 待完成 |
...
followed-thread static relocation/rewrite plan 绑定已 follow 的线程，按 caller-supplied ARM64 bytes、source 和 destination 生成 target-specific code-cache layout。它除 fallback/island 外还返回 `relocatedByteCount`、`hasContinuation`、`continuationAddress/Target/ByteCount`、`controlFlowClosed=true` 和 `continuationMode`；非终止末条追加 `B sourceEnd`，末条 `B/BR/RET` 由自身闭合，`BL/BLR` 在 LR 保留实现前明确拒绝。plan 保持 `planOnly=true`、`targetThreadInstructionRewrite=false`、`rewritesTargetMemory=false`、`rewriteReady=false`、`executionReady=false`，不写目标内存。

`Stalker.prepareTargetThreadRewrite(thread, bytes, start[, options])` 复用 followed-thread 校验，分配双向 direct-branch 可达的 near mapping，按真实 mapping base 发射 target-specific code cache，完成 flush+RX transition，并返回 QuickJS-owned `StalkerCodeCache`。owner 保存完整 prepared source snapshot，报告 `controlFlowClosed=true`、对应 `continuationMode` 与 `sourceSnapshotVerified=false`，并提供幂等 `rollback()`/`dispose()`；准备层不暂停线程、不改写目标地址。

`Stalker.preflightTargetThreadRewrite(thread, cache)` 把 commit 前置检查结构化为 `commit-blocker-report`：要求 cache 来自 prepare 且线程仍被 follow，报告 cache executable、线程暂停/目标 patch/quiescence/双向 branch-range 能力和 blocker 数组。Apple 构建探测 Mach thread suspend 原语，host 构建继续显式报告 `apple-thread-suspend`。prepare 会围绕 source 扫描近地址 hint，对 `mmap` 实际返回地址逐个校验并立即释放远地址结果；近区间耗尽时稳定报告 `target-thread-branch-range`。preflight 只读状态，并报告 `controlFlowClosed=true / sourceSnapshotVerified=false`。
...
hook-engine quiescence primitive 通过 Stalker preflight 暴露编译期能力：启用 native hook engine 时等待 in-flight callbacks 与 generated thunks，stub/host 构建继续保留 `target-thread-quiescence` blocker。`Stalker.commitTargetThreadRewrite()` 在 suspend/quiescence 后比较完整 prepared source snapshot，再应用 4-byte entry branch、flush 并 resume；成功 owner 平铺 continuation 元数据并报告 `targetThreadInstructionRewrite=true / rewritesTargetMemory=true / rewriteReady=true / executionReady=true / controlFlowClosed=true / sourceSnapshotVerified=true`，rollback 在释放 RX cache 前恢复原 entry bytes。后续仍需 Apple runtime/真机验证 hint 分配、`MAP_JIT`、W^X、真实线程执行和 rollback 时序。
