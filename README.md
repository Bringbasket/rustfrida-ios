...
- 顶层 `hook` 现在也新增 `backendMatrix`，会把 controller/target 两侧探测到的 backend 统一收敛成结构化矩阵（`entries / sharedBackendIds / controllerOnlyBackendIds / targetOnlyBackendIds / loadedIn* / filesystemOnly*`），脚本可以直接按 backend 可见性和来源做分支，不必再手动 merge 两侧 `hookEnvironment.backends`。
