...
- `native.dylinker <module>` 现在也已接到 CLI / REPL / `--command-json`；结构化结果会带 `dylinker / path / name / kind / hasPath`，并补 `hasName / pathKind / isTokenPath / usesLoaderPath / usesExecutablePath / usesRpathToken / pathDepth / isWeakDylinker / isReexportDylinker / isUpwardDylinker / isLoadDylinker`，适合直接确认某个 Mach-O 记录的 dyld linker 路径和装载语义。
- `native.installName <module>` 现在也已接到 CLI / REPL / `--command-json`；结构化结果会带 `installName / path / name / currentVersion / compatibilityVersion / timestamp`，并补 `hasName / hasPath / pathKind / isTokenPath / usesLoaderPath / usesExecutablePath / usesRpathToken / pathDepth / hasTimestamp / versionMismatch`，适合快速确认某个 dylib 自身声明的 install name、路径类别和版本信息。
