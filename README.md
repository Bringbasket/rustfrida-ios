...
- `native.uuid <module>` 现在也已接到 CLI / REPL / `--command-json`；结构化结果会带 `imageUuid / uuid`，并补 `normalizedUuid / uuidLength / uuidSegmentCount`，适合把运行中镜像和 dSYM / 本地 Mach-O 做快速 UUID 对齐。
