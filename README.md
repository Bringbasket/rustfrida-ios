...
- `native.dyldInfo / native.linkedit / native.functionStarts / native.codeSignature / native.dataInCode / native.exportsTrie / native.chainedFixups` 这组较重的单项查询现在也补了统一顶层状态字段和关键计数，例如 `has* / resolved / commandName / tableCount / startCount / blobKind / entryCount / segmentCount / importCount`；其中 `native.dataInCode / native.exportsTrie` 现在也会把内层已聚合好的 `kinds` 直接平铺到顶层，`native.linkedit / native.chainedFixups` 也会把 `tableNames / nonEmptyTableNames / tables / pointerFormats / libOrdinals` 这类已聚合摘要直接抬到顶层，脚本侧先做快速判定、拿摘要数组或按 kind 分组时不必总是深入内层大对象。
