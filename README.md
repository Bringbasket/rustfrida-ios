...
- `objc.classProtocols / objc.protocolProtocols / objc.protocolMethods / objc.protocolProperties` 这几类协议相关查询现在也会在结构化结果里补出 owner 解析状态和顶层摘要，例如 `hasClassInfo / hasProtocolInfo / resolved / resolvedClassPointer / resolvedProtocolPointer / declaredProtocolCount / protocolTotalMethodCount / protocolPropertyCount / ownerHasProtocols / ownerHasMethods / ownerHasProperties`；脚本侧在消费过滤后的协议/方法/属性列表时，不必再额外补一轮 `objc.classInfo / objc.protocolInfo` 才能判断 owner 是否存在以及整体元数据规模。
- `objc.methods / objc.properties / objc.ivars` 这三类 class-owned 列表查询现在也统一补了同一套 owner 摘要字段，例如 `classInfo / hasClassInfo / resolved / resolvedClassPointer / declaredProtocolCount / ownerHasProtocols / ownerHasProperties / ownerHasIvars / ownerHasMethods / ownerIvarCount / ownerTotalPropertyCount / ownerTotalMethodCount`；脚本侧在拿过滤结果的同时，就能直接知道宿主类的大致元数据规模，不必再补一轮 `objc.classInfo`。
