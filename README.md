...
- `swift.typeLayout` 列表结果现在也会额外补 `uniqueModuleCount / moduleNames / typeNames`，以及 `layoutsWithMetadataCount / layoutsWithVtableEntriesCount / layoutsWithWitnessTablesCount` 这类“有多少个类型布局带某类块”的摘要，同时还会带 `metadataEntryCount / vtableEntryCount / witnessTableEntryCount` 这类总条目数，适合脚本先看整体布局分布和类型/模块聚合，再决定是否展开单个类型。
