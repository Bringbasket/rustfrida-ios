...
- `native.buildVersion <module>` 现在也已接到 CLI / REPL / `--command-json`；结构化结果会带 `buildVersion / platform / minOs / sdk / tools`，并补 `hasTools / firstTool / lastTool`，适合直接确认目标镜像的 build platform 和工具链版本。
- `native.dylinker <module>` 现在也已接到 CLI / REPL / `--command-json`；结构化结果会带 `dylinker / path / name / kind / hasPath`，适合直接确认某个 Mach-O 记录的 dyld linker 路径。
- `native.installName <module>` 现在也已接到 CLI / REPL / `--command-json`；结构化结果会带 `installName / path / name / currentVersion / compatibilityVersion / timestamp`，并补 `hasPath / hasTimestamp / versionMismatch`，适合快速确认某个 dylib 自身声明的 install name 和版本信息。
