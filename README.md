...
- `native.rpaths <module> [-- <query>]` 现在也已接到 CLI / REPL / `--command-json`；结构化结果会带 `rpaths / path`，并额外补 `pathKind / isTokenPath / usesLoaderPath / usesExecutablePath / usesRpathToken / pathDepth` 这类 entry 摘要，以及 `uniqueRpathCount / uniquePathKindCount / tokenPathCount / loaderPathCount / executablePathCount / rpathTokenCount / longestRpath* / rpathPaths / pathKinds` 这类汇总字段，适合和 `native.dependencies` 一起排查运行时 dylib 查找路径。
