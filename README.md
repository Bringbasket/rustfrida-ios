...
- `swift.protocols / swift.conformances / swift.metadata` 这三类列表结果现在也会额外补 `uniqueModuleCount / uniqueSourceKindCount / sourceDemangledCount / sourceKinds` 这类摘要；其中 `swift.protocols` 还会补 `uniqueProtocolCount / moduleNames / protocolNames`，并继续补 `uniqueContextModuleCount / uniqueDetailKindCount / contextModuleNameList / detailKindList / sourceKindList / contextModules / detailKinds` 这类 protocol descriptor 语义摘要，`swift.conformances` 还会补 `uniqueTypeCount / typeNames / moduleNames / uniqueProtocolCount / protocols`，并继续补 `whereClauseCount / hasWhereClauses / uniqueContextModuleCount / uniqueDetailKindCount / contextModuleNameList / detailKindList / sourceKindList / contextModules / detailKinds` 这类 conformance 语义摘要，`swift.metadata` 还会补 `uniqueTypeCount / moduleNames / typeNames`，并继续补 `uniqueContextModuleCount / uniqueDetailKindCount / contextModuleNameList / detailKindList / sourceKindList / contextModules / detailKinds` 这类 metadata/descriptor 语义摘要，适合脚本先看 Swift 符号来源以及协议/类型/模块分布，而不必自己扫完整数组。
