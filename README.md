...
- `objc.methods` 列表结果现在也会额外补 `uniqueSelectorCount / uniqueReturnTypeCount / keywordSelectorCount / unarySelectorCount / explicitArgumentMethodCount / hiddenArgumentMethodCount / returnsVoidCount / returnsObjectCount / returnsBlockCount / totalExplicitArgumentCount / totalHiddenArgumentCount / maxSelectorPartCount / maxExplicitArgumentCount / selectors / returnTypes` 这类摘要，便于脚本先看某个类的方法分布、selector 形态和返回类型分布，而不必自己扫完整数组再聚合。
- `objc.protocolMethods` 列表结果现在也会额外补 `uniqueSelectorCount / uniqueReturnTypeCount / keywordSelectorCount / unarySelectorCount / explicitArgumentMethodCount / hiddenArgumentMethodCount / returnsVoidCount / returnsObjectCount / returnsBlockCount / totalExplicitArgumentCount / totalHiddenArgumentCount / maxSelectorPartCount / maxExplicitArgumentCount / selectors / returnTypes` 这类摘要，便于脚本先看某个协议方法声明集的 selector 形态和返回类型分布，而不必自己扫完整数组再聚合。
- `objc.classProtocols / objc.protocolProtocols / objc.protocolMethods / objc.protocolProperties` 这几类协议相关查询现在也会在结构化结果里补出 owner 解析状态和顶层摘要，例如 `hasClassInfo / hasProtocolInfo / resolved / resolvedClassPointer / resolvedProtocolPointer / resolvedHasImagePath / resolvedAdoptedProtocolCount / resolvedProtocolTotalMethodCount / resolvedProtocolPropertyCount / resolvedOwnerHas* / declaredProtocolCount / protocolTotalMethodCount / protocolPropertyCount / ownerHasProtocols / ownerHasMethods / ownerHasProperties`；脚本侧在消费过滤后的协议/方法/属性列表时，可以区分“owner 没解析到”和“owner 已解析但值为 0/false”，不必再额外补一轮 `objc.classInfo / objc.protocolInfo`。
- `objc.methods / objc.properties / objc.ivars` 这三类 class-owned 列表查询现在也统一补了同一套 owner 摘要字段，例如 `classInfo / hasClassInfo / resolved / resolvedClassPointer / resolvedHasImagePath / resolvedDeclaredProtocolCount / resolvedOwnerHas* / resolvedOwnerIvarCount / resolvedOwnerTotalPropertyCount / resolvedOwnerTotalMethodCount / declaredProtocolCount / ownerHasProtocols / ownerHasProperties / ownerHasIvars / ownerHasMethods / ownerIvarCount / ownerTotalPropertyCount / ownerTotalMethodCount`；脚本侧在拿过滤结果的同时，就能直接知道宿主类的大致元数据规模，并区分“类不存在”和“类存在但对应项为空”。
...
- `objc.ivars` 列表结果现在也会额外补 `uniqueKindCount / uniqueObjectClassCount / totalQualifierCount / totalObjectProtocolCount / pointerIvarCount / arrayIvarCount / objectIvarCount / blockIvarCount / ivarsWithQualifiersCount / ivarsWithObjectProtocolsCount / ivarsWithObjectClassCount / ivarsWithPointeeTypeCount / ivarsWithMemberNameCount / minOffsetHex / maxOffsetHex / kinds / objectClasses` 这类摘要，便于脚本先看 ivar 布局大致由哪些类型构成、有没有对象/指针/数组，以及偏移大概落在哪个范围。
...
- 在这之上，这组 ObjC 单项查询现在也继续补平了更直接的顶层摘要字段，例如 `resolvedClassPointer / resolvedProtocolPointer / resolvedMethodPointer / resolvedPropertyPointer / resolvedIvarPointer / resolvedImp / resolvedOffsetHex / resolvedReturnTypeName / getterName / setterName / ivarName / argumentCount / hiddenArgumentCount / objectProtocolCount / qualifierCount`；脚本侧读单条 ObjC 结果时不必再反复解包内层 `classInfo / methodInfo / propertyInfo / ivarInfo`。
