...
- `native.hookenv` / `Native.detectHookEnvironment()` 现在除了 backend / warning，还会补出面向当前 `hook_policy` 的建议动作，便于真机上快速判断该走 query-only、cleanup-only、fail-fast 还是继续冒险装 inline hook；返回里也会区分 `allowed` 和 `inlineHooksAllowed`，不再把“允许注入做查询”和“允许安装 inline hook”混成一个布尔值；同时还会额外带上 `conflictState / riskLevel / loadedBackendCount / filesystemOnlyBackendCount / loadedImageCount / filesystemPathCount / coexistenceLayerAvailable`，以及 `bootstrapInjectionAllowed / queryCommandsAllowed / hookInstallCommandsAllowed / hookStatusCommandsAllowed / hookStopCommandsAllowed` 这组能力位，脚本可以直接按命令类别分支，不必再从 warning 文本反推当前冲突态。
