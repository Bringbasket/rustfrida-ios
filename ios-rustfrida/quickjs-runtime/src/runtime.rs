    ...
    mod tests {
        ...
        fn agent_helpers_route_basic_queries() {
            ...
            assert_eq!(
                runtime
                    .eval(
                        "(function() { const result = __iosRustFridaAgentApi.handleSpecResult({ kind: 'objc.protocol_properties', protocolName: 'NSObject', filter: 'description' }); return result.kind === 'objc.protocol_properties' && result.protocolName === 'NSObject' && result.filter === 'description' && result.count === result.properties.length && (result.properties.length === 0 || (typeof result.properties[0].typeEncoding === 'string' && typeof result.properties[0].typeName === 'string' && Array.isArray(result.properties[0].objectProtocols) && typeof result.properties[0].typeInfo === 'object' && typeof result.properties[0].attributeInfo === 'object' && typeof result.properties[0].isReadwrite === 'boolean' && typeof result.properties[0].isAtomic === 'boolean' && typeof result.properties[0].isStrong === 'boolean' && typeof result.properties[0].isCopy === 'boolean' && typeof result.properties[0].isWeak === 'boolean' && typeof result.properties[0].isAssign === 'boolean' && typeof result.properties[0].hasAccessorNames === 'boolean' && typeof result.properties[0].hasOwnershipModifier === 'boolean' && typeof result.properties[0].hasTypeInfo === 'boolean' && typeof result.properties[0].hasObjectClassName === 'boolean' && typeof result.properties[0].hasObjectProtocols === 'boolean' && typeof result.properties[0].hasParsedTokens === 'boolean')) && result.text === result.properties.map((property) => property.text).join('\\n'); })()"
                    )
