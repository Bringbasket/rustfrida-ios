    ...
    mod tests {
        ...
        fn agent_helpers_route_basic_queries() {
            ...
            assert_eq!(
                runtime
                    .eval(
                        "(function() { const main = __iosRustFridaAgentApi.handleSpecResult({ kind: 'native.main_image' }); if (main.image === null) { return true; } const result = __iosRustFridaAgentApi.handleSpecResult({ kind: 'native.dyld_info', moduleName: main.image.name }); return result.kind === 'native.dyld_info' && (result.dyldInfo === null || (typeof result.dyldInfo.commandName === 'string' && typeof result.dyldInfo.commandRequiresDyld === 'boolean' && typeof result.dyldInfo.rebaseOffHex === 'string' && typeof result.dyldInfo.rebaseEndHex === 'string' && typeof result.dyldInfo.hasRebaseInfo === 'boolean' && typeof result.dyldInfo.bindEndHex === 'string' && typeof result.dyldInfo.hasBindInfo === 'boolean' && typeof result.dyldInfo.weakBindEndHex === 'string' && typeof result.dyldInfo.hasWeakBindInfo === 'boolean' && typeof result.dyldInfo.lazyBindEndHex === 'string' && typeof result.dyldInfo.hasLazyBindInfo === 'boolean' && typeof result.dyldInfo.exportSizeHex === 'string' && typeof result.dyldInfo.exportEndHex === 'string' && typeof result.dyldInfo.hasExportInfo === 'boolean' && typeof result.dyldInfo.hasAnyBindInfo === 'boolean' && typeof result.dyldInfo.regionCount === 'number' && typeof result.dyldInfo.totalSizeHex === 'string')); })()"
                    )
