...
/*
 * Best-effort code pointer normalization for arm64e/PAC environments.
 *
 * The current hook engine does not emit authenticated BRAA/BLRAA sequences yet.
 * To keep generated thunks and inline patches usable on more arm64e setups,
 * normalize code pointers before embedding them in generated instructions.
 *
 * On Apple arm64e, function pointers often carry PAC bits in the top address
 * range. Keeping the low 48 bits matches the canonical user-space VA range used
 * by current iOS/macOS processes and avoids embedding signed pointers directly
 * into MOVZ/MOVK or literal loads.
 *
 * This is intentionally conservative and is not a full PAC-aware branch/auth
 * implementation.
 */
static inline uint64_t hook_normalize_code_u64(uint64_t value) {
...
static inline void* hook_normalize_code_ptr(void* value) {
    return (void*)(uintptr_t)hook_normalize_code_u64((uint64_t)(uintptr_t)value);
}

static inline void hook_writer_put_thunk_entry_pac(Arm64Writer* w) {
#if defined(__APPLE__) && defined(__aarch64__)
    arm64_writer_put_paciasp(w);
#else
    (void)w;
#endif
}

static inline void hook_writer_put_thunk_return(Arm64Writer* w) {
#if defined(__APPLE__) && defined(__aarch64__)
    arm64_writer_put_retaa(w);
#else
