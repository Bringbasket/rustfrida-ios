...
void* hook_install(void* target, void* replacement, int stealth) {
    target = hook_normalize_code_ptr(target);
    replacement = hook_normalize_code_ptr(replacement);
...
void emit_save_hook_context(Arm64Writer* w, uint64_t target_pc, uint64_t trampoline_ptr) {
    target_pc = hook_normalize_code_u64(target_pc);
    trampoline_ptr = hook_normalize_code_u64(trampoline_ptr);
...
void emit_callback_call(Arm64Writer* w, HookCallback callback, void* user_data) {
    callback = (HookCallback)hook_normalize_code_ptr((void*)callback);
    ...
    /* Call callback */
    arm64_writer_put_ldr_reg_u64(w, ARM64_REG_X16, hook_normalize_code_u64((uint64_t)(uintptr_t)callback));
...
int hook_attach(void* target, HookCallback on_enter, HookCallback on_leave, void* user_data, int stealth) {
    target = hook_normalize_code_ptr(target);
    on_enter = (HookCallback)hook_normalize_code_ptr((void*)on_enter);
    on_leave = (HookCallback)hook_normalize_code_ptr((void*)on_leave);
