...
void arm64_writer_put_paciasp(Arm64Writer* w) {
    /* PACIASP encodes as HINT #25 on assemblers that do not decode FEAT_PAuth. */
    arm64_writer_put_insn(w, 0xD503233F);
}

void arm64_writer_put_autiasp(Arm64Writer* w) {
    /* AUTIASP encodes as HINT #29 on assemblers that do not decode FEAT_PAuth. */
    arm64_writer_put_insn(w, 0xD50323BF);
}
...
void arm64_writer_put_retaa(Arm64Writer* w) {
    arm64_writer_put_insn(w, 0xD65F0BFF);
}

void arm64_writer_put_retab(Arm64Writer* w) {
    arm64_writer_put_insn(w, 0xD65F0FFF);
}
