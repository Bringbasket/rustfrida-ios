...
pub fn native_export_support_available() -> bool {
    platform::native_export_support_available()
}

pub fn find_image_exports(module_name: &str, query: Option<&str>) -> Result<Vec<NativeSymbol>> {
    platform::find_image_exports(module_name, query)
}
    ...
    pub fn find_image_exports(module_name: &str, query: Option<&str>) -> Result<Vec<NativeSymbol>> {
        ...
        if trimmed_module.is_empty() {
            return Err(Error::InvalidArgument(
                "module name must not be empty; use `native.exports <module>` or `native.exports <module> -- <query>`"
                    .into(),
            ));
        }
        ...
        collect_exports_in_image(&image, trimmed_query)
    }

    fn collect_exports_in_image(image: &ImageInfo, query: Option<&str>) -> Result<Vec<NativeSymbol>> {
...
mod platform {
    ...
    pub fn find_image_exports(module_name: &str, _query: Option<&str>) -> Result<Vec<NativeSymbol>> {
        ...
        Err(Error::Unsupported(
            "native export enumeration is currently only available on Apple targets".into(),
        ))
    }
}
...
#[cfg(test)]
mod tests {
    use super::{find_image_exports, query_matches_symbol};

    #[test]
    fn export_query_matches_plain_and_underscored_names() {
    ...
    #[cfg(not(any(target_os = "ios", target_os = "macos")))]
    #[test]
    fn non_apple_export_enumeration_reports_unsupported() {
        let err =
            find_image_exports("libsystem_malloc.dylib", None).expect_err("non-apple platforms should be unsupported");
